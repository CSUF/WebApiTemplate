		...
		public static void EnrichFromRequest(IDiagnosticContext diagnosticContext, HttpContext context)
		{
			...
			if (context != null)
			{
				CustomEnricherHttpContextInfo theInfo = new CustomEnricherHttpContextInfo()
				{
					...
					ResponseStatusCode = Convert.ToString(context.Response.StatusCode)
				};
				...
				if (!string.IsNullOrWhiteSpace(theInfo.ResponseContentType)) diagnosticContext.Set(nameof(theInfo.ResponseContentType), theInfo.ResponseContentType);
				if (!string.IsNullOrWhiteSpace(theInfo.ResponseStatusCode)) diagnosticContext.Set(nameof(theInfo.ResponseStatusCode), theInfo.ResponseStatusCode);
